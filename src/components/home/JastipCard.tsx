...
export function JastipCard({ user, timeAgo, status, route, tags, deadline, notes, actionText, onClick, onWhatsApp, imageUrl, onStatusChange, onDelete, isOwner }: JastipCardProps) {
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="jcard-top flex justify-between items-start mb-3">
          ...
          <div className={`status-pill inline-flex items-center gap-1 py-1 px-3 rounded-full text-[10px] font-bold tracking-wide uppercase shrink-0 ${isAvailable ? 'bg-sage-pale text-sage-dark' : 'bg-gold-pale text-gold-dark'}`}>
          ...
          <div className="flex gap-2 ml-auto">
            ...
            {!isOwner && (
              <>
                ...
                <button className="btn btn-sm rounded-full text-[12px] font-semibold border-[1.5px] border-charcoal-30 text-charcoal py-2 px-4 hover:border-charcoal hover:bg-charcoal-10 transition-colors">
                  {actionText}
                </button>
              </>
            )}
